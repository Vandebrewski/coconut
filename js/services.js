...
.service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL){
	...
	this.getRecentPosts = function(page) {
		...
		$http.jsonp("http://www.new-impulse.com/tag/health/?json=get_posts&callback=JSON_CALLBACK")
...
.service('PostService2', function ($rootScope, $http, $q, WORDPRESS_API_URL2){
	...
	this.getRecentPosts = function(page) {
		...
		$http.jsonp("http://www.new-impulse.com/tag/beauty/?json=get_posts&callback=JSON_CALLBACK")
		.success(function(data) {
			deferred.resolve(data);
		})
		.error(function(data) {
